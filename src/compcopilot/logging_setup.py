...
def configure_logging(level: str = "INFO", *, json_output: bool = True) -> None:
    """Configure the ``compcopilot`` logger tree.
    ...
    """
    logger = logging.getLogger("compcopilot")
...
def get_logger(name: str) -> logging.Logger:
    """Return a logger namespaced under ``compcopilot``."""
    return logging.getLogger(name if name.startswith("compcopilot") else f"compcopilot.{name}")
